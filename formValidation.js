...
submit.addEventListener('click', (e) => {
    ...
    if (regex.test(str) && regex1.test(str1)) {
        let alertBox = document.createElement('div');
        alertBox.id = 'alert';
        alertBox.setAttribute('style', 'background:skyblue;height:30px;width:95.35%;color:green;position:fixed;top:0px;margin:0px;padding:20px;border-bottom:2px solid green;text-align:center;font-size:20px')
        let alertTxt = document.createTextNode('Your form has been submitted');
        ...
        // alert('Fill All The * Marked Entry Or Some X<--ERROR->X Occured');
        // console.log('some error');
        let alertBox = document.createElement('div');
        alertBox.id = 'alert';
        alertBox.setAttribute('style', 'background:pink;height:30px;width:95.35%;color:red;position:fixed;top:0px;margin:0px;padding:20px;border-bottom:2px solid red;text-align:center;font-size:20px')
        let alertTxt = document.createTextNode('Fill all the * marked entity or some error occured');
        alertBox.appendChild(alertTxt);
        let alert = document.getElementById('divalert');
        alert.appendChild(alertBox);
        setTimeout(removeAlert,2000);
